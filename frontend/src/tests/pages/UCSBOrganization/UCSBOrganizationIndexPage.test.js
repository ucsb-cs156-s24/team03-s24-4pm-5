...
describe("UCSBOrganizationIndexPage tests", () => {
    ...
    test("renders three UCSBOrganizations correctly for regular user", async () => {
        ...
        await waitFor(() => { expect(screen.getByTestId(`${testId}-cell-row-0-col-orgCode`)).toHaveTextContent("CD"); });
        expect(screen.getByTestId(`${testId}-cell-row-1-col-orgCode`)).toHaveTextContent("HK");
        expect(screen.getByTestId(`${testId}-cell-row-2-col-orgCode`)).toHaveTextContent("AS");
        ...
        const orgCode = screen.getByText("CD");
        expect(orgCode).toBeInTheDocument();

        const orgTranslationShort = screen.getByText("Coder");
        expect(orgTranslationShort).toBeInTheDocument();
        const orgTranslation = screen.getByText("CoderSB");
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        expect(axiosMock.history.delete[0].url).toBe("/api/UCSBOrganization");
        // expect(axiosMock.history.delete[0].params).toEqual({ orgCode: "CD"});
    });
